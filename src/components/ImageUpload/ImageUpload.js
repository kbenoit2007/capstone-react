...
function ImageUpload({ bookDetailsData, onBookDetailsClickNext }) {
  ...
  const handleUpload = () => {
    ...
    if (selectedFile) {
      ...
      axios
        .post(
          `${process.env.REACT_APP_URL}:${process.env.REACT_APP_PORT}/upload`,
          formData,
          config
        )
        .then((data) => {
          ...
          navigate(
            `../user/${bookDetailsData.user_id}/viewbook/${bookDetailsData.id}`
          );
        })
        .catch((error) => {
    ...
    <>
      <div>
        ...
        <div className="imageUpload__actionButtons">
          ...
          <div>
            <button onClick={handleUpload} className="rounded-button">
              Upload
            </button>
          </div>
        </div>

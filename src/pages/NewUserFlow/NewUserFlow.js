...
function NewUserFlow(){
    ...
    useEffect(() =>{
      axios.get(`${process.env.REACT_APP_URL}:${process.env.REACT_APP_PORT}/user/new`)
      .then(res => {
         console.log(res)
    ...
    <div className="mainApp__container">
        {activePage === 'SelectEvent' && <SelectEvent onNextClick={handleNextClick}  />}
        {activePage === 'NewBorn' && <NewBorn selectEventData={firstStepData} newBornData={handleNewBornClick} />}
        {activePage === 'TellUsMore' && <TellUsMore newBornData={newBornData} tellUsMoreData={handleTellUsMoreClick} />}

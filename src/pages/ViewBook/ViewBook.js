...
function ViewBook() {
    ...
    useEffect(()=>{
     ...
     useEffect(()=>{
        axios.get(`${process.env.REACT_APP_URL}:${process.env.REACT_APP_PORT}/user/${userId}/viewbook/${bookId}`).then((data)=>{
  ...
  return (
    <div className="mainApp__container">
      ...
      <Slider {...settings}>
            ...
            {images.map((image) => (
              <div className="card">
                <img src={image.url} className="w-100" width="100%" height="500px"/>
                <h3>Milestone</h3>
                <p>text</p>
              </div>
            ))}
